const createFig = () => {
  ...
  return {
    parse ({ bytes, pako, kiwi }) {
      ...
      const chunks = []
      let offset = 12
      console.log(String.fromCharCode(...bytes.slice(0, 12)), bytes.length)
      ...
      try {
        console.log('schemaP', schemaP)
        if (!schemaP) {
          schemaP = kiwi.compileSchema(kiwi.decodeBinarySchema(bytes))
          console.log('chunk1', schemaP)
        } else {
          console.log('chunck12', schemaP, schemaP.decodePixsoMsg(bytes))
        }
      } catch (e) {
        console.log(e)
        return
