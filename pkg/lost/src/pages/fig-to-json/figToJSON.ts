import { ByteBuffer, decodeBinarySchema, compileSchema } from './lib/kiwi/index.js'
...
export const getFigJsonData = (fileBuffer: Buffer) => {
  ...
  while (start < fileByte.length) {
    const end = calcEnd(fileByte, start)
    console.log('start-end', start, end)
    start += 4

    const byteTemp = fileByte.slice(start, start + end)
    console.log('byteTemp:', byteTemp)
