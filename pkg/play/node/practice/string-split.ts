...
const s4 =
  '<p className="asdfasdf zxc2134" onclick={asdf} style="margin-bottom: 0px;">sad</p>'
const s5 =
  '<p style="margin-bottom: 0px;" className="asdfasdf" onclick={asdf} >sad</p>'
const split1 = (s: string) => {
  ...
  realP =
    realP.substring(0, begin + 2) + styleContent + realP.substring(end + 1)
  console.log(realP)
}
//split1(s1)
//split1(s2)
...
const split2 = (s: string) => {
  ...
  realP =
    realP.substring(0, stylePos + 7) +
    'opacity: 0;' +
    realP.substring(stylePos + 7)
  console.log(realP)
}
//const split2 = (s:string) => {
//let realP = s
