...
const BufferContent = () => {
  ...
  const handleDelete =
    (key: string): React.MouseEventHandler<HTMLDivElement> =>
    (e) => {
  ...
  return (
    <>
      <StyledBufferList>
        {bufferList.map((t) => {
          return (
            <StyledBuffer key={t.key} $isactive={t.key === activeBuffer?.key}>
              <span onClick={onClick(t.key)}>{t.key}</span>
              <div className="del-btn" onClick={handleDelete(t.key)}>
                x
              </div>
        ...
        <div className="add-btn" onClick={handleNewBuffer}>
          +
        </div>
      </StyledBufferList>
      <BufferEditor
        dv={activeBuffer?.content || ''}
        bufferKey={activeBuffer?.key}
      />
      {/* <Editor/> */}
    </>
  )
